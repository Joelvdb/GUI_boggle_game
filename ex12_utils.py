...
def find_length_n_paths(n, board, words):
    ...
    for row in range(BOARD_SIZE) :
        for col in range(BOARD_SIZE) :
            _creat_pathes_from_point(board, row, col, n, [], all_paths, words)
    return all_paths
...
def _creat_pathes_from_point(board, row, col, n, cur_path, all_paths, words) :
    ...
    for coor in new_coors :
        if _creat_pathes_from_point(board, coor[0], coor[1], n-1, cur_path[:], all_paths, words) :
            break
...
def find_length_n_words(n, board, words):
    ...
    for row in range(BOARD_SIZE):
        for col in range(BOARD_SIZE):
            _find_words_from_point(board, row, col, n, '', all_paths, words, [])
    return all_paths
